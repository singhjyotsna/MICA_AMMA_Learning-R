...
test <- c(T,T,T,F,F,T,F,F,T,T)
v2 <- matrix(v.logic,nrow = 2,byrow = T)

v2 <- matrix(test,nrow = 2,byrow = T)
...
df1 = data.frame(n, s, b,stringsAsFactors =TRUE)
class (df1$b)
mode (df1$b)

?data.frame
df2<-data.frame(n,s1,b,stringsAsFactors = F)
class(df2$s1)
class(df1$s)
class(df$b)
...
women_result<- subset(women,women$height>mean.h & weight <mean.w)
women_result<- subset(women,height>mean.h & weight <mean.w)
...
city_table_final <-html_table (city_table_final)
view(city_data)
...
View (normal_income)
setwd("C:\\Users\\jyots\\AppData\\Local\\Temp\\RtmpSaw7Wl\\downloaded_packages")
library('Rcmdr')
